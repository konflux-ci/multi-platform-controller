...
func (pw IBMPowerDynamicConfig) doesInstanceHaveTaskRun(instance *models.PVMInstance, log logr.Logger, existingTaskRuns map[string][]string) bool {
	validTaskRunId := false

	// Iterate over this instance's tags
	for _, tag := range instance.UserTags {
		...
		// Return true if the associated exists in a TaskRun namespace
	...
	if !validTaskRunId {
		log.Error(fmt.Errorf("no valid TaskRun IDs were found; deleting instance anyway"), "instanceID", instance.PvmInstanceID)
	}
	return validTaskRunId
}
