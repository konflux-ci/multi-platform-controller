...
// GetState returns ibmz's VM state from the IBM VPC service. See https://cloud.ibm.com/apidocs/vpc/latest?code=go#get-instance for
// valid state strings.
func (ibmz IBMZDynamicConfig) GetState(kubeClient client.Client, ctx context.Context, instanceId cloud.InstanceIdentifier) (string, error) {
	vpcService, err := ibmz.authenticatedService(ctx, kubeClient)
	if err != nil {
		return "", fmt.Errorf("failed to create an authenticated VPC service: %w", err)
	}
